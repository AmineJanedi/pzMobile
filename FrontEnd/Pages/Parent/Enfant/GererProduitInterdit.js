...
const GererProduitInterdit = () => {
  ...
  useEffect(() => {
    const fetchProduits = async () => {
      try {
        const response = await axios.get('http://192.168.1.4:4000/ProduitInterdit/getProduitInterdit');
  ...
  const handleChercherProduit = async (text) => {
    try {
      const response = await axios.get(`http://192.168.1.4:4000/ProduitInterdit/chercherProduitInterdit/${text}`);
      console.log(response.data);
      if (response.data && Array.isArray(response.data)) {
        // Mise à jour de l'état produits avec les nouveaux produits trouvés
        setProduits(response.data.map(produit => ({ ID: produit.ID, nom: produit.NomProduit })));
      } else {
  ...
  const handleClick = async (ID) => {
    try {
      const response = await axios.delete(`http://192.168.1.4:4000/ProduitInterdit/DeleteProduitInterdit/${ID}`);
  ...
  <Text style={[styles.listeDesProduits, styles.bonjourNourredinTypo]}>
    ...
    borderRadius:19,
        ...
        {chercher === '' ? (
          produits && Array.isArray(produits) && produits.map((produit, index) => (
            <View key={index} style={{ flexDirection: 'row' }}>
              <Text style={{ right: -19, top: 5, marginRight: 20, marginBottom: 21 }}>{`ID : ${produit.ID}`}</Text>
              <Text style={{ right: -19, top: 5, marginRight: 40, marginBottom: 21 }}>{`Nom : ${produit.NomProduit}`}</Text>
              <TouchableOpacity onPress={() => handleClick(produit.ID)}>
                <Image
          ...
          produits.filter(produit => produit.NomProduit.toLowerCase().includes(chercher.toLowerCase())).map((resultat, index) => (
            <View key={index} style={{ flexDirection: 'row' }}>
              ...
              <Text style={{ right: -19, top: 5, marginRight: 40, marginBottom: 21 }}>{`Nom : ${resultat.NomProduit}`}</Text>
      ...
      <Pressable
        ...
        onPress={() => navigation.navigate("AjouterProduitInterditParent")}
      />
      <Text
...
export default GererProduitInterdit;
